...
const getAllServices = asyncHandler(async (req, res) => {
  ...
  //Count the total number of products
  // const count = await Product.countDocuments({ ...keyword })
  const count = await Product.countDocuments({ brand: "other" });

  //Limit the products sent to UI based on the pagesize
  //skip is for showing next set of products on the next page etc.
  // const products = await Product.find({ ...keyword })
  const products = await Product.find({ brand: "other" })
    .sort({ name: 1 })
    .limit(pageSize)
    .skip(pageSize * (page - 1));
...
const getSaiServices = asyncHandler(async (req, res) => {
  ...
  const products = await Product.find({ brand: "baba" }).sort({ name: 1 });
